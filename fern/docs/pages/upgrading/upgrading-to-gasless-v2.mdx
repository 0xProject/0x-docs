...
<Tip>
  ...
  [contact support](/docs/need-help/contact-support#contact-support).
</Tip>
...
- improved [monetization features](/docs/swap-api/guides/monetize-your-app)
...
- new [buy/sell tax support](/docs/swap-api/additional-topics/buy-sell-tax-support)
...
In v2, for [tokens that do not support gasless approvals](/api-reference/openapi-yaml/gasless/getgaslessapprovaltokens), use the [standard token approval flow](/docs/swap-api/additional-topics/how-to-set-token-allowances).

<Note>
When using the [Standard Approval Flow](/docs/swap-api/additional-topics/how-to-set-token-allowances) (aka Gasless Approvals are not supported)

- NEVER set an allowance on the [Settler contract](/docs/core-concepts/0-x-cheat-sheet#0x-settler-contracts). Doing so may result in unintended consequences, including potential loss of tokens or exposure to security risks. The Settler contract does not support or require token allowances for its operation. Setting an allowance on the Settler contract will lead to misuse by other parties.

- ONLY set allowances on [AllowanceHolder](/docs/core-concepts/0-x-cheat-sheet#allowanceholder-contract) or [Permit2](/docs/core-concepts/0-x-cheat-sheet#permit2-contract) contracts, as indicated by the API responses.
...
- [ ] [Get a 0x API key](../../docs/introduction/quickstart/create-a-0-x-api-key)
