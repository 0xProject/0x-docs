...
| `bridge_failed` | The bridge did not complete, but recovery may still be in progress. Check `failure.status` before you stop polling. See [Handling Failures and Recovery](/cross-chain/cross-chain-api/key-concepts/handling-failures-and-recovery). | ❌ |
...
Keep polling while an automatic refund or 0x dest retry is in progress. Dest-retry hashes can appear in `transactions` before the funds arrive. Only `bridge_filled` confirms delivery.

If `manualTransaction` is present, the recipient must sign it. `/status` does not track that transaction. See [Handling Failures and Recovery](/cross-chain/cross-chain-api/key-concepts/handling-failures-and-recovery) for details.
