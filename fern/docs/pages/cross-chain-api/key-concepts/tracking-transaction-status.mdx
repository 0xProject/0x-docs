...
| `bridge_failed` | The bridge did not complete. Keep polling while `failure.status` is `refund_pending` or `manual_action_required`. A dest retry that lands becomes `bridge_filled`. See [Handling Failures and Recovery](/cross-chain/cross-chain-api/key-concepts/handling-failures-and-recovery). | ❌ |
...
When `status` is `bridge_failed`, the response includes a `failure` object with the failure reason and recovery status. For dest retries 0x submits, `manualTransaction` is omitted. Keep polling. Submit `manualTransaction` only when it is present.
