...
| `bridge_failed` | The bridge did not complete. Keep polling while `failure.status` is `refund_pending`, or `manual_action_required` with `manualTransaction` omitted. A dest retry that lands becomes `bridge_filled`. If `manualTransaction` is present, the recipient signs it. See [Handling Failures and Recovery](/cross-chain/cross-chain-api/key-concepts/handling-failures-and-recovery). | ❌ |
...
When `status` is `bridge_failed`, the response includes a `failure` object with the failure reason and recovery status. Dest retries 0x submits omit `manualTransaction`. Dest-retry hashes can appear in `transactions` and `failure.transactions` before fill. Filled means `status` is `bridge_filled`. If `manualTransaction` is present, the recipient must sign it. That is a dest claim, not a dest retry. `/status` does not follow that tx.
