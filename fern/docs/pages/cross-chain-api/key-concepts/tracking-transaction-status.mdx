...
When `status` is `bridge_failed`, the response includes a `failure` object with the failure reason and recovery status. Dest retries 0x submits omit `manualTransaction`. Keep polling. If `manualTransaction` is present, the recipient must sign it. That is a dest claim, not a dest retry.
