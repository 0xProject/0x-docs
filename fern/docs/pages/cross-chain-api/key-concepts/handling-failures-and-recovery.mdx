...
| --- | --- | --- |
| `chainId` | number | Chain where recovery occurs. Destination retries use the destination chain. Refunds use the origin chain, except for Mayan MCTP and FastMCTP refunds, which use the destination chain. |
| `token` | string | Token recovered or delivered. A destination retry delivers the bridge step's `buyToken`. An origin refund returns the bridge step's `sellToken`. Mayan MCTP and FastMCTP destination refunds return USDC. The token may differ from the quote's `sellToken`. |
