...
| `"Fee collection is not supported when origin chain is Tron"` | Fees are supported on EVM and Solana origins only. Remove `feeBps` / `feeRecipient` / `feeToken` for Tron quotes. |
| `"Solana origins support a maximum of 2 integrator fees"` | Reduce `feeBps` / `feeRecipient` to at most 2 entries. |
| `"Invalid Solana address format for feeRecipient"` | Use a base58 Solana pubkey. |
| `"Invalid Solana token address format for feeToken"` | Use a base58 Solana mint address. |
