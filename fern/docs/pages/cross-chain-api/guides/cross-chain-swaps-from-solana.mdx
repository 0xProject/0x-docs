...
Solana is rolling out [transaction format v1](https://solana.com/upgrades/larger-transaction-sizes). The official guide covers activation timing, supported libraries, and the full migration details.
...
`serializedTransaction` remains base64 encoded for both versions. Make sure your client can deserialize, sign, and send v1 before submitting the transaction. Use `@solana/kit` 8+ or `@solana/web3.js` 3.x. `@solana/web3.js` 1.x cannot send v1.
