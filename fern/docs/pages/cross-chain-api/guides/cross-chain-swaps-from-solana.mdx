...
In our example, we will be swapping WSOL on Solana to USDC on Base. For understanding how to deal with the native SOL, please refer to [Native Tokens Handling](/cross-chain/cross-chain-api/key-concepts/native-tokens-handling).
...
Decode `serializedTransaction` from base64. Quotes are v0 today. Use a library that can also sign and send v1, so a later v1 quote does not fail. `@solana/web3.js` 1.x cannot send v1. See [Solana transaction v1](/svm/solana-swap-api/guides/solana-transaction-v1).
...
Solana activates format v1 on mainnet at epoch 1035, expected September 15, 2026. Cross-Chain quotes stay v0 today. Keep decoding `serializedTransaction` from base64.

If you call `getTransaction` or `getBlock` yourself, pass `maxSupportedTransactionVersion: 1` as the JSON integer `1`. Passing `0` fails on a v1 transaction with error `-32015`. Tracking the swap with `/status` does not require that RPC call.

See [Solana transaction v1](/svm/solana-swap-api/guides/solana-transaction-v1) for libraries, `transactionConfig`, and the 4096-byte cap.
