...
<Info>
  Unlike EVM chains, Tron cross-chain swaps do not require token approvals. The
  API uses deposit-only transactions, so there is no `allowanceTarget` and no
...
const triggerResponse = await fetch(
  `${TRON_RPC_URL}/wallet/triggersmartcontract`,
  {
    method: "POST",
