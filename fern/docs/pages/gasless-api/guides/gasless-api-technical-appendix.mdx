...
To take advantage of gaslesses approvals and gasless trades, user must sign the `approval.eip712` and the `trade.eip712` objects returned by [`/quote`](/api-reference/evm-ap-is/gasless/getquote). These objects contain everything (domain, types, primaryType, message) needed when signing these objects .
...
If a token supports [gasless approvals](/evm/gasless-api/guides/understanding-gasless-api#gasless-approvals), the `/quote` response will return an “approval” object which contains the necessary information to process a gasless approval, see below:
...
`approval.type` tells you which permit ABI the token uses. Sign `approval.eip712` exactly as returned in every case — the value only affects how Gasless API submits the approval on-chain, not how you produce the signature.

| `approval.type` | Standard | Notes |
| --- | --- | --- |
| `permit` | [EIP-2612](https://eips.ethereum.org/EIPS/eip-2612) | The standard permit, with a split `(v, r, s)` signature. |
| `erc7597Permit` | [ERC-7597](https://eips.ethereum.org/EIPS/eip-7597) | Permit with a `bytes` signature. Also supports smart contract wallet signatures via [ERC-1271](https://eips.ethereum.org/EIPS/eip-1271). |
| `daiPermit` | Non-standard | The DAI-style permit, which approves an unlimited allowance rather than a specific amount. |
| `executeMetaTransaction::approve` | Non-standard | Meta-transaction approval used by tokens on Polygon PoS and similar bridged deployments. |

<Note>

If you branch on `approval.type`, handle `erc7597Permit` alongside `permit`. Tokens that only expose the `bytes` signature permit — and takers using smart contract wallets — otherwise fall back to a gasful on-chain approval.
