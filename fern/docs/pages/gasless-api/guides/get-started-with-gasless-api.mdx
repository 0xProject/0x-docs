...
- `approval`: This is the “approval” object which contains the necessary information to process a gasless approval. See the [quote response](/api-reference/evm-ap-is/gasless/getquote#option-1) for an example.
  ...
  - There isn't a separate list of tokens that support gasless approvals to check ahead of time — whether `approval` is `null` in the quote response is the source of truth.
...
There isn't a static list of tokens that support gasless approvals. Instead, check the `approval` object in the [quote](/api-reference/evm-ap-is/gasless/getquote) response for a given token — if it's not `null`, gasless approval is available.
