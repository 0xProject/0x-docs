...
Make sure to read [Understanding Gasless API](/evm/gasless-api/guides/understanding-gasless-api) for an in-depth understanding of how it works, key terms, and technical flow charts for how to implement in-app.
...
![CreateAppScreen](../../../assets/img/introduction/0x-dashboard-create-app-screen.png)

## 1. Fetch Price

Use [`/gasless/price`](/api-reference/evm-ap-is/gasless/getprice) to get the indicative price. This is used when the user is just _browsing_ for the price they could receive on the specified asset pair, but they are not ready to submit the quote yet.

This endpoint responds with pricing information, but the response does not contain a full 0x order, so it does not constitute a full transaction that can be submitted to the Ethereum network (you must use [`/gasless/quote`](/api-reference/evm-ap-is/gasless/getquote) for this). Think of `/price` as the the "read-only" version of `/quote`.
...
When the user has found a price they are happy with and are ready to fill a quote, they should request a firm quote from Gasless API using the [`/gasless/quote`](/api-reference/evm-ap-is/gasless/getquote) endpoint.
...
- `approval`: This is the “approval” object which contains the necessary information to process a gasless approval. See the [quote response](/api-reference/evm-ap-is/gasless/getquote#option-1) for an example.
  ...
  - Gasless approvals work only for tokens that support [EIP-2612](https://eips.ethereum.org/EIPS/eip-2612) or a related permit ABI, such as [ERC-7597](https://eips.ethereum.org/EIPS/eip-7597) or the non-standard DAI permit. Check the `approval` object in the [quote](https://docs.0x.org/api-reference/evm-ap-is/gasless/getquote) response — if it's not `null`, gasless approval is available for that token, and `approval.type` tells you which permit flavor it uses. See [Gasless Approval Tokens](#gasless-approval-tokens) for the full list of values.
...
When user accepts the quote and wants to submit the trade, use [`/gasless/submit`](/api-reference/evm-ap-is/gasless/submit) to send both signatures back along with the payload from `/gasless/quote`.
...
Once the user has signed, split, and submitted both signatures - gasless approval (if applicable) and gasless trade - and a `tradeHash` is returned from `/submit`, you can poll [`/gasless/status/{tradeHash}`](/api-reference/evm-ap-is/gasless/getstatus) to check the status of the trade.
...
If you receive an error response, see the [`/status` response in the Gasless API reference](/api-reference/evm-ap-is/gasless/getstatus)

Once the trade is successful, you can display the transaction confirmation to your user.

## Gasless Approval Tokens

Gasless approvals work only for tokens that support [EIP-2612](https://eips.ethereum.org/EIPS/eip-2612) or a related permit ABI, such as [ERC-7597](https://eips.ethereum.org/EIPS/eip-7597) or the non-standard DAI permit. Check the `approval` object in the [quote](https://docs.0x.org/api-reference/evm-ap-is/gasless/getquote) response — if it's not `null`, gasless approval is available for that token, and `approval.type` tells you which permit flavor it uses.

`approval.type` can be any of the following. Sign `approval.eip712` exactly as returned in every case — the value only affects how Gasless API submits the approval on-chain, not how you produce the signature. See [Sign gasless approval object](./gasless-api-technical-appendix#sign-gasless-approval-object) for details.

| `approval.type` | Standard |
| --- | --- |
| `permit` | [EIP-2612](https://eips.ethereum.org/EIPS/eip-2612) |
| `erc7597Permit` | [ERC-7597](https://eips.ethereum.org/EIPS/eip-7597) — permit with a `bytes` signature, which also supports smart contract wallets via [ERC-1271](https://eips.ethereum.org/EIPS/eip-1271) |
| `daiPermit` | Non-standard DAI-style permit |
| `executeMetaTransaction::approve` | Non-standard meta-transaction approval |
