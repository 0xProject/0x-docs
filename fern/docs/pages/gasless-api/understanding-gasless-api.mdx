...
[ERC-7597](https://eips.ethereum.org/EIPS/eip-7597) is a permit variant that takes the signature as a single `bytes` value instead of the split `(v, r, s)` values used by EIP-2612:
...
A 65-byte `(v, r, s)` tuple can only carry an ECDSA signature from an EOA, so smart contract wallets — Safe, smart accounts, and [EIP-7702](https://eips.ethereum.org/EIPS/eip-7702)-delegated EOAs — cannot use the EIP-2612 permit at all. The `bytes` form accepts [ERC-1271](https://eips.ethereum.org/EIPS/eip-1271) contract signatures, so those wallets can approve gaslessly too. USDC is the most widely used token that supports it.

Gasless API returns `approval.type` of `erc7597Permit` only after verifying on-chain that the bytes-signature permit path succeeds for that token. If your integration ignores this value, smart contract wallet takers fall back to a standard on-chain approval and pay gas for it.
