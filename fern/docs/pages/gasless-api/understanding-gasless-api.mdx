---
title: Understanding Gasless API
...
For tokens that do not support gasless approvals, such as native tokens, you need to set the token allowance via the steps listed [here](/evm/0x-swap-api/additional-topics/handling-native-tokens).
...
[EIP-2612](https://eips.ethereum.org/EIPS/eip-2612) unlocked gasless token approvals by introducing a new Permit function to ERC-20 tokens. The Permit function enables users to grant permission to others to spend their tokens in a single transaction, which makes token approvals more efficient and enables gasless transactions.

### ERC-7597: Permit Extension for Smart Contract Wallets

Used when a token's `permit` function takes the signature as a single `bytes` value instead of the split `(v, r, s)` values that [EIP-2612](https://eips.ethereum.org/EIPS/eip-2612) uses. Gasless API reports these tokens with an `approval.type` of `erc7597Permit`.

```solidity
// EIP-2612
permit(owner, spender, value, deadline, uint8 v, bytes32 r, bytes32 s)

// ERC-7597
permit(owner, spender, value, deadline, bytes signature)
```

A 65-byte `(v, r, s)` tuple can only carry an ECDSA signature from an EOA, so smart contract wallets — Safe, smart accounts, and [EIP-7702](https://eips.ethereum.org/EIPS/eip-7702)-delegated EOAs — cannot use an EIP-2612 permit at all. The `bytes` form accepts [ERC-1271](https://eips.ethereum.org/EIPS/eip-1271) contract signatures, so those wallets can approve gaslessly too. USDC is the most widely used token that supports it.

Gasless API returns `erc7597Permit` only after verifying on-chain that the bytes-signature permit path succeeds for that token.
...
Settler Metatransactions, `settler_metatransaction`, represents a type of gasless transactions returned by the Gasless API [`/gasless/quote`](/api-reference#tag/Gasless/operation/gasless::getQuote) endpoint. Settler Metatransaction leverages the [Swap API](/api-reference#tag/Swap) to fetch indicative pricing and quotes. 0x submits these transactions to the blockchain on behalf of the user.
...
[Click here to expand the image](../../assets/img/gasless-api/gasless-and-swap-flow-chart.jpg)

![Composing Gasless Approval + Gasless Swaps + Swap API Flow Chart](../../assets/img/gasless-api/gasless-and-swap-flow-chart.jpg)
