...
## Accessing RFQ Liquidity

This guide is for integrators who want to access 0x RFQ liquidity via the Swap API. If you represent a trading firm or professional market maker looking to _supply_ RFQ liquidity, see the [0x RFQ Interest Form](/liquidity-integration/liquidity-integrations#rfq-liquidity-request).

<Note>
  RFQ access requires upgrading from the Standard tier to the Customer tier.
  [Fill out this form](https://0x.org/contact) to get in touch with the 0x
  team.
</Note>
...
![RFQ Diagram](../../../assets/img/0x-swap-api/rfq-diagram.png)
...
RFQ liquidity is included in a quote when `taker` is included in a [`/quote` request](/api-reference/evm-ap-is/swap/allowanceholder-getquote).
