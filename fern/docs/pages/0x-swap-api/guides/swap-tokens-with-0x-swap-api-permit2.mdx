...
Use the [`/swap/permit2/price`](/api-reference/evm-ap-is/swap/permit-2-getprice) endpoint to get an indicative price for an asset pair. This returns pricing information without creating a full order or transaction, allowing the user to browse potential prices before committing.
...
For implementation details, see [how to set your token allowances](/evm/0x-swap-api/additional-topics/how-to-set-your-token-allowances).
...
When you're ready to execute a trade, request a firm quote from the Swap API using [`/swap/permit2/quote`](/api-reference/evm-ap-is/swap/permit-2-getquote).
...
- [(Guide) Build a Next.js App with 0x Swap API](/evm/0x-swap-api/guides/permit2/build-token-swap-dapp-nextjs-permit2)
