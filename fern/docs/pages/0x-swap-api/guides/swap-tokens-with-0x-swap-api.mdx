...
This guide will cover the core steps to using Swap API, specifically using the [Swap AllowanceHolder](/api-reference/evm-ap-is/swap/allowanceholder-getprice) endpoint.
...
Use the [`/swap/allowance-holder/price`](/api-reference/evm-ap-is/swap/allowanceholder-getprice) endpoint to get an indicative price for an asset pair. This returns pricing information without creating a full order or transaction, allowing the user to browse potential prices before committing.
...
const priceParams = new URLSearchParams({
  chainId: "1", // / Ethereum mainnet. See the 0x Cheat Sheet for all supported endpoints: https://docs.0x.org/docs/core-concepts/0x-cheat-sheet
...
For detailed instructions, see [how to set your token allowances](/evm/0x-swap-api/additional-topics/how-to-set-your-token-allowances).
...
When you're ready to execute a trade, request a firm quote from the Swap API using [`/swap/allowance-holder/quote`](/api-reference/evm-ap-is/swap/allowanceholder-getquote).
...
<Tip>
  To verify that a quote came from 0x and was not altered in transit, request a signed response by adding an `Accept-Signature` header. See [Response Signatures](/docs/developer-resources/response-signatures).
</Tip>
...
- [(Guide) Build a Next.js App with 0x Swap API](/evm/0x-swap-api/guides/build-token-swap-dapp-nextjs)
- [(Guide) Verify Response Signatures](/docs/developer-resources/response-signatures)
