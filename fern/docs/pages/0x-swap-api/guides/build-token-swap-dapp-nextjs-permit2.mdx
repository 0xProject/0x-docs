...
If you're new to 0x Swap API or want to avoid the double-signature UX, consider using [AllowanceHolder](/evm/0x-swap-api/guides/build-token-swap-dapp-nextjs) instead.
...
This guide is intended for advanced integrators only. If you are new to Swap API or want to avoid the double-signature flow, we recommend using the Swap API [AllowanceHolder](/evm/0x-swap-api/guides/build-token-swap-dapp-nextjs) flow.
...
![](../../../assets/img/0x-swap-api/swap-v2-demo-app.png)
...
- [viem](https://viem.sh/)
- Swap API concepts ([intro](/evm/0x-swap-api/introduction) or [video](https://www.youtube.com/watch?v=APXjSGLaoRw))
...
PriceView displays indicative pricing using[/swap/allowance-holder/price](/api-reference/evm-ap-is/swap/permit-2-getprice). Indicative pricing is for users to browse for a price without committing to a trade.
...
![](../../../assets/img/0x-swap-api/swap-demo-price-view.gif)
...
![token allowance ui](../../../assets/img/0x-swap-api/token-allowance-ui.png)
...
QuoteView displays a firm, executable quote returned from [/swap/allowance-holder/quote](/api-reference/evm-ap-is/swap/permit-2-getquote).
...
![swap demo quoteview](../../../assets/img/0x-swap-api/swap-demo-quoteview-affiliate-fee.png)
...
For implementation details and pricing considerations, see the [how to monetize your app using 0x Swap API](/evm/0x-swap-api/guides/monetize-your-app-using-swap).
...
By following the best practices outlined in this blog post, you can create a user-friendly and effective app that enables trustless token swapping on our [supported chains](/docs/introduction/supported-chains).
