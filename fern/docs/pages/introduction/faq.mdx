...
<AccordionGroup>
  ...
  <Accordion title="Why does my 0x transaction revert?">
    ...
    <ul>
      <li>Has a [token allowance](/evm/0x-swap-api/additional-topics/how-to-set-your-token-allowances) been properly set for the user to trade the <code>sellToken</code>?</li>
      <li>Does the user have enough <code>sellToken</code> balance to execute the swap?</li>
      <li>Does the user have enough to pay the gas?</li>
      <li>Is the slippage tolerance too low?</li>
      <li>Is the token liquidity too shallow to be swapped? </li>
  ...
  <Accordion title="Did my order revert because the RFQ quote expired?">
    ...
    <ul>
      ...
      <li>You may need to reach out to <a href="https://docs.0x.org/docs/introduction/need-help#-contact-developer-support-fastest-direct-help">0x support</a> to help decode <code>expiryAndNonce</code>.</li>
    </ul>
...
<Accordion title="I received `BUY_TOKEN_NOT_AUTHORIZED_FOR_TRADE` or `SELL_TOKEN_NOT_AUTHORIZED_FOR_TRADE`. Help!">
  ...
  <Accordion title="I received an `INTERNAL_SERVER_ERROR` error. Help!">
    ...
    <ul>
      ...
      <li>If the issue persists, <a href="https://docs.0x.org/docs/introduction/need-help#-contact-developer-support-fastest-direct-help">contact support</a>.</li>
    </ul>
  ...
  <Accordion title="How will I know if the trade is not possible due to insufficient asset liquidity?">
    ...
    <br /><br />
    <img src="../../assets/img/developer-resources/liquidityAvailable.png" alt="liquidityAvailable parameter" />
...
<AccordionGroup>
  <Accordion title="What is the difference between using AllowanceHolder and Permit2 for Swap API?">
    ...
    Endpoints:
    <ul>
      <li><a href="/api-reference/evm-ap-is/swap/allowanceholder-getprice">/swap/allowance-holder/price</a></li>
      <li><a href="/api-reference/evm-ap-is/swap/allowanceholder-getquote">/swap/allowance-holder/quote</a></li>
    </ul>
    ...
    <ul>
      <li><code>0x0000000000001fF3684f28c67538d4D072C22734</code> — **Cancun hardfork chains** (Ethereum Mainnet, Arbitrum, Avalanche, Base, Berachain, BSC, Ink, Linea, Monad, Optimism, Plasma, Polygon, Scroll, Sonic, Unichain, World Chain)</li>
      <li><code>0x0000000000005E88410CcDFaDe4a5EfaE4b49562</code> — **Shanghai hardfork chains** (Mantle)</li>
    </ul>
    ...
    Endpoints:
    <ul>
      <li><a href="/api-reference#tag/Swap/operation/swap::permit2::getPrice">/swap/permit2/price</a></li>
      <li><a href="/api-reference#tag/Swap/operation/swap::permit2::getQuote">/swap/permit2/quote</a></li>
    </ul>
...
<Accordion title="Has the AllowanceHolder contract been formally verified?">
  Yes. On top of the <a href="https://github.com/0xProject/0x-settler/tree/master/audits">audits</a> covering 0x Settler, <code>AllowanceHolder</code> has been independently <strong>formally verified</strong> by Bailsec using the Certora Prover. Formal verification mathematically proves the contract satisfies a written specification across <em>every</em> reachable state and execution path, rather than only the paths that testing and fuzzing happen to explore.

  The specification covers 50+ properties across five verification targets,
  including:
  <ul>
    <li>
      <strong>Allowance accounting</strong>: <code>exec</code> sets exactly the
      granted allowance, <code>transferFrom</code> moves exactly the requested
      amount and decrements the allowance by that amount, and allowances can
      never increase outside those two paths.
    </li>
    <li>
      <strong>No passive approvals</strong>: allowances live entirely in
      EIP-1153 transient storage, and an invariant proves{" "}
      <code>AllowanceHolder</code> never writes persistent storage — so no
      allowance can outlive the transaction that granted it.
    </li>
    <li>
      <strong>Operator isolation</strong>: only the operator named in the grant
      can consume it; a non-operator caller cannot spend another user's
      allowance.
    </li>
    <li>
      <strong>Confused-deputy protection</strong>: calls whose target looks like
      an ERC-20 are proven to revert, and the forwarded <code>msgSender</code>{" "}
      recovery is proven to return the original caller.
    </li>
    <li>
      <strong>Fallback routing</strong>: <code>AllowanceHolder</code> dispatches
      through a hand-written assembly fallback — each selector is proven to
      reach the correct branch, the branches are proven mutually exclusive, and
      unknown selectors are proven to revert.
    </li>
    <li>
      <strong>No stuck ETH</strong>: no ether is retained by the contract after{" "}
      <code>exec</code> or <code>transferFrom</code>, including callback flows.
    </li>
  </ul>
  Read the full <a href="https://github.com/0xProject/0x-settler/blob/master/audits/Bailsec%20AllowanceHolder%20FV/Bailsec_0x_%E2%80%93_AllowanceHolder_%E2%80%93_Formal_Verification_1st_FV_Report.pdf">AllowanceHolder Formal Verification report</a> or browse the <a href="https://github.com/0xProject/0x-settler/tree/master/audits/Bailsec%20AllowanceHolder%20FV/certora">Certora specs</a> used to produce it.
</Accordion>
...
<Accordion title="How does Exact Buy handle token allowances and trade surplus?">
...
<Accordion title="How can I get the ABI for an ERC-20 contract via TypeScript?">
  ...
  <Accordion title="What’s the best way to access 0x trade data?">
    ...
    <ul>
      <li><a href="/evm/trade-analytics-api/introduction">0x Trade Analytics API</a> — historical analysis, usage tracking, reporting.</li>
      <li><a href="/evm/0x-swap-api/additional-topics/0x-parser">0x-parser</a> — TypeScript library for decoding real-time 0x transaction logs.</li>
    </ul>
  ...
  <Accordion title="Can I detect whether a transaction originated from the 0x API by parsing transaction receipts?">
    ...
    <ul>
      <li>Use the <a href="/evm/trade-analytics-api/introduction">0x Trade Analytics API</a>.</li>
    ...
    <ul>
      ...
      <li>For aggregated usage insights, <a href="https://docs.0x.org/docs/introduction/need-help#-contact-developer-support-fastest-direct-help">reach out</a> and share your use case.</li>
    </ul>
...
<AccordionGroup>
  <Accordion title="Does 0x create ATAs if we don't have one, specifically for our fee address?">
...
<Accordion title="Whose data is returned?">
  ...
  <Accordion title="Can I detect whether a transaction originated from the 0x API by parsing transaction receipts?">
    ...
    **🔍 If you're trying to track your own app's activity**
    ...
    <ul>
      <li>The <a href="/evm/trade-analytics-api/introduction">0x Trade Analytics API</a> is the best place to start. It allows you to query detailed trade data associated with your API key.</li>
...
<AccordionGroup>
  <Accordion title="Are the smart contracts audited?">
    Yes. See <a href="https://github.com/0xProject/0x-settler/tree/master/audits">audit reports</a>. The <code>AllowanceHolder</code> contract has additionally been <a href="https://github.com/0xProject/0x-settler/blob/master/audits/Bailsec%20AllowanceHolder%20FV/Bailsec_0x_%E2%80%93_AllowanceHolder_%E2%80%93_Formal_Verification_1st_FV_Report.pdf">formally verified</a>. Check out our <a href="/docs/developer-resources/bounties">bounty program</a>.
  </Accordion>
</AccordionGroup>
