...
<AccordionGroup>
  ...
  <Accordion title="Does 0x support Solana transaction v1?">
    Swap instructions and Cross-Chain quotes are still v0. After Solana activates v1, any `getTransaction` or `getBlock` call you make must pass `maxSupportedTransactionVersion: 1`. `@solana/web3.js` 1.x can read v1 from 1.99.0 but cannot sign or send it.

    See [Solana transaction v1](/svm/solana-swap-api/guides/solana-transaction-v1).
  </Accordion>
</AccordionGroup>
