...
<Accordion title="What tokens work with gasless approvals?">
  Gasless approvals are supported only for tokens that implement [EIP-2612](https://eips.ethereum.org/EIPS/eip-2612)
  (`permit`). Rather than checking a static token list, request a
  [quote](https://docs.0x.org/api-reference/evm-ap-is/gasless/getquote) and check
  the [`approval` object](https://docs.0x.org/evm/gasless-api/guides/get-started-with-gasless-api#response-details)
  in the response — if it's not `null`, the token supports gasless approval.
</Accordion>
