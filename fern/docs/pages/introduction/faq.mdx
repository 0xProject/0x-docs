...
<Accordion title="How can I find the tokens that 0x supports for trading?">
  ...
  Note: Balancer V3 pool tokens (BPTs) are not supported by 0x API. Selling a
  BPT is effectively redeeming an LP position, which 0x API does not support —
  it routes trades between tokens rather than entering or exiting liquidity
  pools. To exit a Balancer V3 position, withdraw through Balancer first, then
  swap the underlying tokens through 0x.
</Accordion>
...
<Accordion title="Has the AllowanceHolder contract been formally verified?">
  Yes. The <code>AllowanceHolder</code> contract has undergone multiple external{" "}
...
<AccordionGroup>
  ...
  <Accordion title="Who can I offer RWAs to?">
    You are responsible for restricting access on your platform. The RWA trading agreement requires you to prevent access by users located in the United States, any jurisdiction subject to comprehensive sanctions, any jurisdiction where offering the relevant asset is prohibited by applicable law, and any jurisdiction excluded by the asset's issuer. It also requires you to follow any other restrictions, conditions, or requirements stipulated by the asset's issuer or under applicable law. See <a href="/rwa/introduction#your-responsibilities">Your responsibilities</a> for details and an example.
  </Accordion>
</AccordionGroup>
