...
<Note>
  The 0x Solana Swap API is in **open beta** — [get an API key](https://dashboard.0x.org/create-account) to start building. Interfaces may still change; share feedback in [Discord](https://discord.com/invite/official0x).
</Note>
...
Solana is rolling out [transaction format v1](https://solana.com/upgrades/larger-transaction-sizes). Use that page for activation timing, library versions, and RPC behavior.

This guide still builds a v0 transaction. Stay under the 1232-byte v0 limit.

The Cross-Chain API prefers v0 for compatibility. It may return v1 when a bridge provider sends a v1 transaction, or when the instructions cannot fit in v0.

If you call `getTransaction`, pass `maxSupportedTransactionVersion: 1` as the JSON integer `1`. You do not need `getBlock` for the 0x Swap or Cross-Chain flow.

Decode Cross-Chain `serializedTransaction` from base64. To sign and send v1, use `@solana/kit` 8+ or `@solana/web3.js` 3.x. `@solana/web3.js` 1.x cannot send v1.
