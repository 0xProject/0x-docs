...
<Tip>
  **If you're unsure what value to use for the integrator bytes field, start at zero or keep it as small as possible.** Setting it higher than necessary reduces the quality of routes the API can return. The correct value is best determined experimentally — build your transaction, measure its actual byte cost using the steps below, and set the field to that value.
</Tip>
