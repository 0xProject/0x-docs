...
Solana v0 transactions are limited to **1232 bytes**. v1 raises that cap to 4096 bytes. 0x swap instructions are still compiled as v0, so stay under 1232 unless you are compiling v1 yourself. See [Solana transaction v1](/svm/solana-swap-api/guides/solana-transaction-v1).
