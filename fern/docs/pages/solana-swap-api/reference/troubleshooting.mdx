...
| `getTransaction` returns `-32015`, or `getBlock` fails for the whole slot | A v1 transaction is on chain and the RPC call did not opt in | Pass `maxSupportedTransactionVersion: 1` as the JSON integer `1`. See [Solana transaction v1](/svm/solana-swap-api/guides/solana-transaction-v1) |
