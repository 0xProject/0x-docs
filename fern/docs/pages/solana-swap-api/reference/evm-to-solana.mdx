...
Tokens have an on-chain mint account owned by a *token program*. The 0x Solana Swap API supports both the SPL Token Program and the Token-2022 (Token Extensions) Program, though certain Token-2022 extension configurations are not routable — see [Token Support](/svm/solana-swap-api/guides/important-integration-notes#token-support) for details.
...
A simple, effective way to verify if a token address is a swappable token is to check whether the account is owned by the SPL Token Program or the Token-2022 Program.

To verify whether a token address on Solana is a valid SPL or Token-2022 mint:
...
/**
 * Checks whether a list of token addresses are SPL or Token-2022 tokens.
...
export async function classifyTokenAccounts(
  ...
  for (const address of addresses) {
    try {
      ...
      let tokenType = "NOT an SPL or Token-2022 token";
      if (isSPL) tokenType = "an SPL token";
      else if (isToken2022) tokenType = "a Token-2022 token";

      console.log(`${address} is ${tokenType}`);
    } catch (err) {
