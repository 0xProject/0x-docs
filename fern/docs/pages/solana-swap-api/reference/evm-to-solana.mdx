...
Each SPL token has an on-chain mint account owned by either the SPL Token Program or the Token-2022 (Token Extensions) Program. The 0x Solana Swap API supports both, though certain Token-2022 extension configurations are not routable — see [Token Support](/svm/solana-swap-api/guides/important-integration-notes#token-support) for details.
