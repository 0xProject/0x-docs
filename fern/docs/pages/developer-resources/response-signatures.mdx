...
<Tip>
  See the [signed responses example](https://github.com/0xProject/0x-examples/tree/main/swap-signed-responses-example) for a complete TypeScript implementation that requests and verifies signed EVM quotes, Solana swap instructions and cross-chain quotes.
</Tip>
...
- **Cross-Chain API:** `/cross-chain/quotes`, `/cross-chain/status`, `/cross-chain/sources`

<Note>
  Streaming endpoints such as `/cross-chain/quotes/stream` are never signed. Do not send `Accept-Signature` to them: the response is held until the stream ends.
</Note>
...
- Streaming endpoints, such as `/cross-chain/quotes/stream`.
