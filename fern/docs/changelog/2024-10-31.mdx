...
- **Sell Entire Balance** option is now supported in Swap API v2. To enable, set `sellEntireBalance` to `true`. Supported for both allowance-holder and permit2 endpoints. [Learn more about implenting this feature](/evm/0x-swap-api/additional-topics/sell-entire-balance).
...
- New [guide](/evm/0x-swap-api/additional-topics/handling-native-tokens) on best practices for swapping native tokens (e.g., ETH).
