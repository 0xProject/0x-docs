...
Once you've created a team, you'll be taken to the [0x Dashboard](https://dashboard.0x.org/) where you can create and manage live API keys, find build resources, and raise support requests.

To find a list of all networks supported by 0x, check out the [0x Cheat Sheet](https://0x.org/docs/introduction/0x-cheat-sheet).
...
In the next screen, you can add a unique tag that can be used to identify transaction data from your app on [0x Explorer](https://explorer.0x.org/apps). You may use the same tag for all your apps, or create a separate one for each.
...
```bash
// Replace the API key in the header with your sample key
curl https://api.0x.org/swap/v1/quote?buyToken=0x6B175474E89094C44Da98b954EedeAC495271d0F&sellToken=0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE&sellAmount=100000&excludedSources=Kyber&takerAddress=0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045  --header '0x-api-key: [api-key]'
...
<center>
   <img src={DashboardDemoApp} width="100%"/>
</center>
```

For each App, you can see the following:

1. See which **0x products are enabled for your API key** - Swap API and/or Orderbook API.
2. From **API Key**, **see all the API keys** associated with this app and **create or delete keys**. From **Settings**, change the 0x products enabled for this app. Set your **0x Explorer Tag**. Change the **App Name**.
...
```mdx-code-block
import AccountSettings from '/static/img/introduction/0x-dashboard-settings.png';

<center>
   <img src={AccountSettings} width="40%"/>
</center>
```
...
Add a 0x Explorer Tag to your app via the 0x Dashboard. [0x Explorer](https://explorer.0x.org/?period=day), a public dashboard that provides transaction tracking, liquidity source and chain breakdowns, and in-depth metrics for your apps. It also showcases your apps and their activity to the broader 0x ecosystem and potential users!

![0x Explorer Screenshot](/img/developer-resources/explorer-screenshot.png)

Read more about how to submit your 0x Explorer Tag [here](http://localhost:3000/docs/developer-resources/0x-explorer-tag-guide).
