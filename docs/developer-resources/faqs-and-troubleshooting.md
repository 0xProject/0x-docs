...
- The slippage tolerance may be too low if the liquidity is very shallow for the token the user is trying to swap. Read [here](/0x-swap-api/guides/troubleshooting-0x-api-swaps#slippage-tolerance) for how to handle this.
- Fee-on-transfer tokens may wreak havoc on our contracts. Read [here](/0x-swap-api/guides/troubleshooting-0x-api-swaps#fee-on-transfer-tokens) for how to handle this
