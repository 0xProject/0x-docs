---
sidebar_label: Introduction
sidebar_position: 1
description: An introduction to how the Swap API works and the resources included this section
---

# Introduction

_Easily enable crypto trading in your app with Swap API_

:::tip

Prefer to watch a video or see a code demo instead? Jump to [0x Videos](/introduction/guides#videos) or [0x Code Examples](/introduction/guides#code-examples).

:::

## What is Swap API?

Swap API is a professional-grade DEX aggregation and smart order routing REST API that allows you to find the best price and easily submit the trade with just a few lines of code.

Swap API allows you to add crypto trading in your application with just a single API integration. It finds the best real-time price across 100+ liquidity sources, both public (AMMs) and private (professional market makers), across [9 different chains](/0x-swap-api/introduction#supported-networks)

![Swap API UI](/img/0x-swap-api/swap-api-ui.png)

:::tip

Want to build a gasless trading app? Check out [Gasless API](/docs/tx-relay-api/introduction) instead.

:::

## Why use Swap API?

Swap API watches out for your users, is easy for your developers, and your team can earn fee revenue!

- ⛓️ Easily access **[multi-chain liquidity](/introduction/0x-cheat-sheet#swap-api-endpoints)** on the most popular networks
  - Ethereum, Sepolia, Polygon, BSC, Optimism, Fantom, Celo, Avalanche, Arbitrum, Base ([full list of endpoints](/introduction/0x-cheat-sheet#swap-api-endpoints))
- 🤑 **[Earn revenue fees](/developer-resources/faqs-and-troubleshooting#monetizing-your-swap-integration)** through your app's swap integration
- 🛡️ **Built-in user-protection features**, including **[Price Impact Protection](advanced-topics/price-impact-protection.md)** which **protects** users **from getting rekt** by illiquid markets
- ↩ Best execution with the **lowest reverts**
- 💰 Exclusive built-in **[RFQ liquidity](/0x-swap-api/guides/accessing-rfq-liquidity-on-0x-api)** which is competitive with AMMs and comes with the added benefit of zero slippage and guaranteed MEV protection
- 🚀 It is **easy-to-use!** For example, you can easily find the best price to buy DAI with WETH with the following request:

## Try it out

:::tip

👉 Run this curl request to see a live quote response for selling ETH -> DAI, excluding Kyber as a source:

```bash
// Replace API key in the header with your own key from https://dashboard.0x.org/
// Using vitalik.eth as takerAddress
curl https://api.0x.org/swap/v1/quote?buyToken=0x6B175474E89094C44Da98b954EedeAC495271d0F&sellToken=0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE&sellAmount=100000&excludedSources=Kyber&takerAddress=0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045  --header '0x-api-key: REPLACE_WITH_API_KEY'

```

<br></br>
You will receive a response that looks like this:{" "}

```json
{
    "chainId": 1,
    "price": "1969.101",
    "guaranteedPrice": "1949.40999",
    "estimatedPriceImpact": "0",
    "to": "0xdef1c0ded9bec7f1a1670819833240f027b25eff",
    "data": "0xd9627...
    ...
}
```

This is a valid unsigned Ethereum transaction that can be [submitted directly to a node](/0x-swap-api/guides/swap-tokens-with-0x-swap-api#4-send-the-transaction-to-the-network) to complete the swap. GetRead more about the parameters [here](/0x-swap-api/api-references/get-swap-v1-quote).

<br></br>

**Note:** This request uses a test API key that should not be used in production. Create an account and get your live API keys to access the Swap API from **[0x Dashboard](https://go.0x.org/create-account-txrelay-z)**.

:::

## How does it work?

Swap API is a professional-grade DEX aggregation and smart order routing API. Using the API, developers can easily and reliably tap into aggregated multi-chain DEX liquidity. Swap API finds the best executed price across 70+ liquidity sources, both public (AMMs) and private (professional market makers), across a growing number of blockchains

Below is a high-level diagram showing how Swap API works under the hood:

![Swap API Diagram](/img/introduction/swap-api-diagram.png)

1. Retail Trader Requests a quote
2. Swap API simultaneously (a) retrieves quote from AMM and (b) requests quotes from Market Makers
3. Market Makers responds with signed quote at their discretion
4. Swap API aggregates the quotes it has received and compares the prices, taking into account [price impact](/0x-swap-api/advanced-topics/price-impact-protection). It returns the best price to the Retail Trader.
5. If the Retail Trader likes the quote, the Retail Trader signs the order. Swap API sends the signed order to the blockchain to be settled by the 0x Protocol.

For more details about how 0x orders are executed, check out [How does 0x work?](/introduction/introduction-to-0x#how-does-0x-work).

Under the hood, Swap API performs a series of tasks:

- **Queries prices** from multiple DEXs and market makers and **aggregates the liquidity** from the queried sources to provide the best price possible. Think of how Google flights aggregates flight prices for a certain time and date to help you find the best price, `/swap` similarly helps you find the best price across DeFi liquidity sources.
- Swap API’s **smart order routing** algorithm splits up your transaction across different sources to maximize the overall return on your swap. Read more about smart order routing [here](https://medium.com/@merklejerk/0x-apis-smart-order-routing-7af0195515e5).
- Swap API's **[Price Impact Protection](/0x-swap-api/advanced-topics/slippage-protection)** calculates price impact estimates, and allows developers to easily notify users if insufficient liquidity may negatively affect the price.
- The response returned by Swap API is a valid unsigned Ethereum transaction that can be submitted directly to an Ethereum node. **[Easily execute using the transaction using the web3 library of your choice](/0x-swap-api/guides/swap-tokens-with-0x-swap-api#3-send-the-transaction-to-the-network)**.

## Supported Networks

- Ethereum (Mainnet): [https://api.0x.org/](https://api.0x.org/)
- Ethereum (Sepolia): [https://sepolia.api.0x.org/](https://sepolia.api.0x.org/)
- Arbitrum: [https://arbitrum.api.0x.org/](https://arbitrum.api.0x.org/)
- Avalanche: [https://avalanche.api.0x.org/](https://avalanche.api.0x.org/)
- Base [https://base.api.0x.org/](https://base.api.0x.org/)
- Binance Smart Chain: [https://bsc.api.0x.org/](https://bsc.api.0x.org/)
- Celo: [https://celo.api.0x.org/](https://celo.api.0x.org/)
- Fantom: [https://fantom.api.0x.org/](https://fantom.api.0x.org/)
- Optimism: [https://optimism.api.0x.org/](https://optimism.api.0x.org/)
- Polygon: [https://polygon.api.0x.org/](https://polygon.api.0x.org/)

Read more about using the [Swap API](/0x-swap-api/api-references/overview).

## Get Started

:::info
To create an account, and get your live API keys to access the Gasless, visit the **[0x Dashboard](https://go.0x.org/create-account-txrelay-z)**.
:::

Now that you're familiar with what Swap API offers, read [How to Use Swap API](/0x-swap-api/guides/swap-tokens-with-0x-swap-api) to get started or choose one of the resources below.

#### Code Examples

- [Next.js 0x Demo App (TypeScript)](https://github.com/0xProject/0x-nextjs-demo-app)

#### Guides

- [Get started with Swap API](/0x-swap-api/guides/swap-tokens-with-0x-swap-api)
- [How to Build a Token Swap DApp with Swap API](/0x-swap-api/guides/how-to-build-a-token-swap-dapp-with-0x-api)
- [Use 0x API Liquidity in Your Smart Contracts](/0x-swap-api/guides/use-0x-api-liquidity-in-your-smart-contracts)
- [Working in the Testnet](/0x-limit-orders/guides/working-in-the-testnet)

#### API References

- [API References Overview](/0x-swap-api/api-references/overview)
- [GET /swap/v1/quote](/0x-swap-api/api-references/get-swap-v1-quote)
- [GET /swap/v1/price](/0x-swap-api/api-references/get-swap-v1-price)
- [GET /swap/v1/sources](/0x-swap-api/api-references/get-swap-v1-sources)

#### Advanced Topics

- [Rate Limiting](/0x-swap-api/advanced-topics/rate-limiting)
- [How to Set Your Token Allowances](/0x-swap-api/advanced-topics/how-to-set-your-token-allowances)
- [Price Impact Protection](/0x-swap-api/advanced-topics/price-impact-protection)

**API FAQs**

- [FAQs & Troubleshooting](/developer-resources/faqs-and-troubleshooting)
