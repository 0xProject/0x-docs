...
Be aware that token you want to use for testing _must_ have liquidity on at least one of the liquidity [/sources](/0x-swap-api/api-references/get-swap-v1-sources); otherwise, you will receive an error.
...
Refer to [@0x/contract-addresses](https://github.com/0xProject/protocol/blob/development/packages/contract-addresses/addresses.json#L44-L85) to find the addresses of the latest deployment on Sepolia and then follow our limit order related guides below, using the Sepolia contract addresses, and chain id 5.
