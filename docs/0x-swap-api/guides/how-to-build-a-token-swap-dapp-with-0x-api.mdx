---
sidebar_label: Build a token swap dApp (HTML, CSS, JS)
sidebar_position: 2
description: An end-to-end tutorial that shows you how to build a token swapping dApp using the 0x Swap API.
---

# How to build a token swap dApp with Swap API (HTML, CSS, JS)

_Learn how to build a token swapping DApp (a simple [Matcha.xyz](https://matcha.xyz/)) using the 0x [Swap API](/0x-swap-api/introduction) with HTML, CSS, and JS. It aggregates liquidity across the greater DEX ecosystem surfaces the best price to the user._
...
<iframe
  width="560"
  height="315"
  src="https://www.youtube.com/embed/tVvZ1ivp4X0"
...
- [Create a 0x account and get a live API key](https://dashboard.0x.org/apps)
...
:::info
📣 Starting July 2023, all requests to Swap API now require API keys. Legacy code examples may contain requests without API keys. Follow [this guide](/introduction/getting-started) for how to get a live API key and use it for any 0x products.
:::
...
```html
<div class="modal" id="token_modal" tabindex="-1" role="dialog"></div>
```
...
:::info
You won’t find ETH in this list because it’s not actually an ERC20 token. Needs to be wrapped, WETH. [Learn more about WETH](https://cointelegraph.com/news/what-is-wrapped-ethereum-weth-and-how-does-it-work). Some apps have a curated list rather than showing all options.
:::
...
```html
< !-- Replace the SELECT A TOKEN text --> < !-- From token -->
<div class="swapbox_select token_select" id="from_token_select">
...
Now we need to create a function called `getPrice()`. Inside, we will be calling the [GET /swap/v1/price endpoint](/0x-swap-api/api-references/get-swap-v1-price).
...
:::tip
This API endpoint is using the Ethereum (mainnet) as indicated by `https://api.0x.org`. Here is a list of [all API endpoints for 0x supported networks](/0x-swap-api/api-references/overview).
:::
...
Read more about setting token allowances [here](/0x-swap-api/advanced-topics/how-to-set-your-token-allowances).
...
The reason we can directly pass in the [`/quote` response](/0x-swap-api/api-references/get-swap-v1-quote) is because it contains all the necessary [parameters for web3.eth.setTransaction()](https://web3js.readthedocs.io/en/v1.2.11/web3-eth.html#id84) - _from, to, value, gas, data, etc_.
...
:::info
This tutorial is part of [Alchemy's Road to Web3 series](https://docs.alchemy.com/docs/welcome-to-the-road-to-web3), a 10-week, self-paced web3 developer program to help new blockchain devs go from beginner to advanced. Check out the original tutorial [here](https://docs.alchemy.com/docs/how-to-build-a-token-swap-dapp-with-0x-api).

<br />
<br />

In order to receive your Proof of Knowledge NFT for this week, share your experience working on the challenges in the [Alchemy University Discord](https://discord.com/invite/xVebBUBxXk).
