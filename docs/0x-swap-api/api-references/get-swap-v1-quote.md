...
RFQ-T liquidity can only be accessed if the request supplies a taker address, a permissioned API key via header `0x-api-key`, and the query parameter `intentOnFilling=true`. For more details see the guide [here](/0x-swap-api/guides/accessing-rfq-liquidity-on-0x-api).
