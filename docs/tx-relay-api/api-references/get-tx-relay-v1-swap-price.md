...
| `acceptedTypes`                   |  \[optional] Comma delimited string of the types of order the caller is willing to receive. <br></br><br></br> Currently, `metatransaction_v2` and `otc` are supported and allowed. More details about order types are covered in [/quote documentation](https://0x.org/docs/tx-relay-api/api-references/get-tx-relay-v1-swap-quote). <br></br><br></br>  This is useful if the caller only wants to receive types the caller specifies. _**If not provided, it means the caller accepts any types whose default value is currently set to `metatransaction_v2` and `otc`**_. |
